...
var BoxTorso = BodyPart.extend({
    ...
    */
	...
	addToStage: function(stage, METER) {
		...
		var TEETH_TOP_ROOT_WIDTH_FACTOR =
			MOUTH_CORNER_WIDTH_FACTOR *
			(1 - TEETH_TOP_REDUCTION_FACTOR) /
			(1 - Math.pow(TEETH_TOP_REDUCTION_FACTOR, TEETH_TOP_NUM))

		var TEETH_BOT_NUM = this.props.width * 2
		var TEETH_BOT_REDUCTION_FACTOR = 0.9
		var TEETH_BOT_ROOT_HEIGHT_FACTOR = 0.4
		var TEETH_BOT_ROOT_WIDTH_FACTOR =
			MOUTH_BOTTOM_WIDTH_FACTOR *
			(1 - TEETH_BOT_REDUCTION_FACTOR) /
			(1 - Math.pow(TEETH_BOT_REDUCTION_FACTOR, TEETH_BOT_NUM))
		...
		var mouthTopX = this.props.width * METER,
			...
			mouthCornerY =
				this.props.height * METER * (1 - MOUTH_CORNER_HEIGHT_FACTOR),
			mouthBottomX_1 =
				this.props.width * METER * (1 - MOUTH_BOTTOM_WIDTH_FACTOR),
		...
		var mouth_top_cp1_X = mouthTopX,
			...
			mouth_top_cp2_X =
				mouthCornerX + this.props.width * METER * MOUTH_CORNER_WIDTH_FACTOR / 2,
		...
		for (var i = 0; i < TEETH_TOP_NUM; ++i) {
			curToothWidth =
				this.props.width *
				METER *
				TEETH_TOP_ROOT_WIDTH_FACTOR *
				Math.pow(TEETH_TOP_REDUCTION_FACTOR, i)
			curToothHeight =
				this.props.height *
				METER *
				TEETH_TOP_ROOT_HEIGHT_FACTOR *
				Math.pow(TEETH_TOP_REDUCTION_FACTOR, i)
		...
		for (var i = 0; i < TEETH_BOT_NUM; ++i) {
			curToothWidth =
				this.props.width *
				METER *
				TEETH_BOT_ROOT_WIDTH_FACTOR *
				Math.pow(TEETH_BOT_REDUCTION_FACTOR, i)
			curToothHeight =
				this.props.height *
				METER *
				TEETH_BOT_ROOT_HEIGHT_FACTOR *
				Math.pow(TEETH_BOT_REDUCTION_FACTOR, i)
		...
		graphics.pivot = new PIXI.Point(
			this.props.width * METER / 2,
			this.props.height * METER / 2
		)
