...
var RevoluteJoint = BodyPart.extend({
  ...
  */
	...
	init: function(initialJointData) {
		if (typeof initialJointData === 'undefined') {
			initialJointData = {}
		}
		...
		var junctions = [
			...
			new Mind.EfferentJunction(this, function() {
				var sum = 0
				this.impulseQueue.forEach(function(value) {
					sum += value
				})
		...
		for (var key in initialJointData) {
			if (typeof initialJointData[key] !== 'undefined') {
				this.initialJointData[key] = initialJointData[key]
			}
		}
	...
	setInitialBodyValues: function(attachA, attachB) {
		...
		var A_anchor_world = attachA.bodyPart.body.GetWorldPoint(
			attachA.complement.anchorPoint
		)
		var A_angle = attachA.bodyPart.body.GetAngle()
		attachB.bodyPart.initialAngle =
			A_angle + this.initialJointData.referenceAngle
		var B_anchor_rot = Utils.Box2D.b2Math.RotV(
			attachB.complement.anchorPoint,
			attachB.bodyPart.initialAngle
		)
	...
	addToWorld: function(world) {
		...
		if (
			attachA.bodyPart != null &&
			attachB.bodyPart != null &&
			attachA.complement != null &&
			attachB.complement != null
		) {
			if (attachA.bodyPart.body !== null && attachB.bodyPart.body === null) {
				...
				attachA.bodyPart.body === null &&
				attachB.bodyPart.body !== null
			) {
			...
			// a joint definition.
			var world_anchor = attachA.bodyPart.body.GetWorldPoint(
				attachA.complement.anchorPoint
			)

			var jointDef = new Box2D.Dynamics.Joints.b2RevoluteJointDef()
			jointDef.Initialize(
				attachA.bodyPart.body,
