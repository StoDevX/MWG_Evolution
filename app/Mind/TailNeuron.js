...
var TailNeuron = Neuron.extend({
    ...
    */
		...
		var distanceDendrite = function(dist) {
			if (dist > this.props.distanceThreshold) {
				// Not yet in range
	...
	computeMotorSpeeds: function() {
		// return true if the speeds were all calculated correctly
		...
		for (var i = 0; i < this.axons.length; ++i) {
			// only need to compute speeds that are actually output
			...
			var curAngle = Utils.Math.signedRemainder(
				this.currentJointAngles[i],
				2 * Math.PI
			)
			var targetAngle = Utils.Math.signedRemainder(
				this.props.keyPositions[this.props.targetPosition][i],
				2 * Math.PI
			)
