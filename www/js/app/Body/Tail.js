    ...
    */
		...
		for (var i = 0; i < this.props.numVertebrae; ++i) {
			var vertebra = new Vertebra(
				{
					width:
						this.props.rootWidth * Math.pow(this.props.widthReductionFactor, i),
					height:
						this.props.rootHeight *
						Math.pow(this.props.heightReductionFactor, i),
					density:
						this.props.rootDensity *
						Math.pow(this.props.densityReductionFactor, i),
		...
		for (var i = 0; i < this.props.numVertebrae; ++i) {
			var joint = new RevoluteJoint({
				...
				motorSpeed: 0,
				maxMotorTorque:
					this.props.rootMaxTorque *
					Math.pow(this.props.torqueReductionFactor, i),
			})
			this.joints[i] = joint
		}
