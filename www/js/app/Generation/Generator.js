...
var Generator = Class.extend({
		...
		for (var i = 0; i < num; i++) {
			var TData = {
				torsoData: {
					...
					height: Utils.Math.randRange(0.1, 2),
					density: Utils.Math.randRange(1, 5),
					friction: 0.01,
				},

				leftWheelData: {
					radius: Utils.Math.randRange(0.2, 1.5),
					density: 2,
				...
				rightWheelData: {
					radius: Utils.Math.randRange(0.1, 1.3),
					density: 2,
				...
				tailData: {
					...
					rootWidth: Utils.Math.randRange(0.1, 0.7),
					rootHeight: Utils.Math.randRange(0.5, 1.3),
					rootDensity: Utils.Math.randRange(0.5, 1.5),
					rootMaxTorque: Utils.Math.randRange(75000, 750000),
					widthReductionFactor: Utils.Math.randRange(0.85, 1.25),
					heightReductionFactor: Utils.Math.randRange(0.7, 1.2),
					densityReductionFactor: Utils.Math.randRange(0.8, 1.1),
					torqueReductionFactor: Utils.Math.randRange(0.9, 1.3),
					friction: 0.5,
				},

				tailNeuronData: {
					maxMotorSpeeds: [
						Utils.Math.randRange(1, 5),
						Utils.Math.randRange(5, 20),
					],
	...
	Generate1: function(parentA, parentB) {
		...
		for (var prop in BData) {
			for (var key in BData[prop]) {
				// Important: AData and BData should have the same property types contained in them.
				if (Math.random() < 0.5) {
					CData[prop][key] = BData[prop][key]
				}
				if (
					Math.random() < this.mutationRate &&
					typeof CData[prop][key] === 'number'
				) {
				...
				if (Math.random() < this.mutationRate && key == 'maxMotorSpeeds') {
					...
					CData[prop][key] = [
						Utils.Math.randRange(AData[prop][key][0], BData[prop][key][0]),
						Utils.Math.randRange(AData[prop][key][1], BData[prop][key][1]),
					]
				}
			}
		}
